...
<body>
	...
	<div class="jumbotron">
		<div class="container">
			...
			작성자 :${dto.writer} 작성일 : ${dto.writeDate} 조회수 :${dto.readCnt } <br>
			<br>
			<h2>${dto.content}</h2>
			<p>
				<%-- 좋아요 싫어요 : 나중에 ajax 처리 --%>
			<form action="good.final">
			...
			<form action="bad.final" style="position: absolute; right: 15px;">
	...
	<c:if test="${not empty dto.up_adress}">
		<br>
		첨부파일 
		<a href="download.final?num=${dto.num}">${dto.up_adress}</a>
		<br>
	</c:if>
					...
					<td>댓글작성자 :${comment.writer}<br> 내용 : ${comment.content}
						<div align="right">
							<c:if test="${login.grade==0 || comment.writer==login.id}">
								<form action="deleteComment.final">
	...
	<c:if test="${not empty login}">
		<form action="writeComment.final">
