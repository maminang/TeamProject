...
<body>
	...
	<c:if test="${not empty login}">
		<form action="writeUI.final2?curPage=${to.curPage}">
				...
				<c:forEach items="${list}" var="article">
					<tr>
						...
						<td><c:forEach begin="1" end="${article.repIndent}">└</c:forEach>
							<a href="read.final2?num=${article.num}&curPage=${to.curPage}">
		...
		<ul class="pagination justify-content-center">
			<li class="page-item">
				<c:choose>
					<c:when test="${to.curPage > 1}">
						<a class="page-link"
							href="list.final2?curPage=${to.beginPageNum-1 > 0?to.beginPageNum-1:1}">Previous</a>
					</c:when>
					<c:otherwise>
			...
			<c:forEach begin="${to.beginPageNum}" end="${to.stopPageNum}"
				varStatus="status">&nbsp;
						...
						<c:otherwise>
							<a class="page-link"
								href="list.final2?curPage=${status.current}">${status.current}</a>
						</c:otherwise>
					</c:choose>
					</li>
			</c:forEach>
			...
			<li class="page-item">
				<c:choose>
					<c:when test="${to.curPage < to.totalPage}">
						<a class="page-link"
							href="list.final2?curPage=${to.stopPageNum+1 < to.totalPage?to.stopPageNum+1:to.totalPage}">Next</a>
					</c:when>
					<c:otherwise>
...
<!-- 	페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징 페이징  -->
		...
		<form action="list.final2?curPage=1">
