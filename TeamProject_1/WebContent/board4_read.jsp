...
<body>
	...
	<div class="jumbotron">
		<div class="container">
			...
			작성자 :${dto.writer} 작성일 : ${dto.writeDate} 조회수 :${dto.readCnt }
			${youtube } <br>
			<br>
			<h2>${dto.content}</h2>
			<p>
				<%-- 좋아요 싫어요 : 나중에 ajax 처리 --%>
			<form action="good.final4">
				<button class="btn btn-outline-primary my-2 my-sm-0"
					style="position: absolute; right: 100px;" type="submit">

					<c:if test="${idGood}">
						<div class="glyphicon glyphicon-heart" style="text-align: center;">
							좋아요${dto.good}
						</div>
					</c:if>
					<c:if test="${!idGood}">
			...
			<form action="bad.final4" style="position: absolute; right: 15px;">
			...
			<c:if test="${login.grade==0 || dto.writer==login.id}">
				<form action="updateUI.final4"
					style="position: absolute; right: 52px; top: 340px;">
			...
			<c:if test="${login.grade==0 || dto.writer==login.id}">
				<form action="delete.final4"
					style="position: absolute; right: 15px; top: 340px; float: right;">
					<input type="hidden" value="${dto.num}" name="num">
	...
	<c:if test="${not empty dto.up_adress}">
		<br>
		첨부파일 
		<a href="download.final4?num=${dto.num}">${dto.up_adress}</a>
		<br>
	</c:if>
	...
	<table class="table table-hover">
		<tbody>
			<c:forEach items="${commentList}" var="comment">
				<tr>
					<td>댓글작성자 :${comment.writer}<br> 내용 : ${comment.content}
						<div align="right">
							<c:if test="${login.grade==0 || comment.writer==login.id}">
								<form action="deleteComment.final4">
									<input type="hidden" value="${dto.num}" name="num"> <input
										type="hidden" value="${comment.cnum}" name="cnum">
									<button id="deleteBtn" class="btn btn-sm btn-primary"
	...
	<c:if test="${not empty login}">
		<form action="writeComment.final4">
