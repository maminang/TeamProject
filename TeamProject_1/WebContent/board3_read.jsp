...
<body>
	...
	<div class="jumbotron">
		<div class="container">
			...
			<form action="bad.final3" style="position: absolute; right: 15px;">
	...
	<c:if test="${not empty dto.up_adress}">
		<br>
		첨부파일 
		<a href="download.final3?num=${dto.num}">${dto.up_adress}</a>
		<br>
	</c:if>
					...
					<td>댓글작성자 :${comment.writer}<br> 내용 : ${comment.content}
						<div align="right">
							<c:if test="${login.grade==0 || comment.writer==login.id}">
								<form action="deleteComment.final3">
	...
	<c:if test="${not empty login}">
		<form action="writeComment.final3">
