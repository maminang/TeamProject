...
public class Board4DAO {
	...
	private Board4DTO getRSInfo(ResultSet rs) {
		Board4DTO dto = null;
		try {
			...
			dto = new Board4DTO(num, title, content, writer, writeDate, readCnt, repRoot, repStep, repIndent, good, bad,
	...
	private int getMaxNum(Connection conn) {
		...
		String sql = "SELECT max(num)+1 FROM tbl_final4";
	...
	private int getAmount(Connection conn) {
		...
		String sql = "SELECT count(num) FROM tbl_final4";
		try {
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public Page4TO listPage(int curPage) {
		Page4TO to = new Page4TO(curPage);
		...
		String sql = "SELECT * FROM "
				...
				+ "(SELECT A.*, "
				+ "(SELECT COUNT(*) FROM tbl_comment_final4 C WHERE C.num = A.num) commentcnt FROM tbl_final4 A ORDER BY reproot DESC, repStep ASC)) "
	...
	public int write(Board4DTO dto) {
		int crtdNum = -1;
		Connection conn = null;
		PreparedStatement pstmt = null;

		String sql = "INSERT INTO tbl_final4 (num,title,content,writer,reproot,repstep,repindent,good,bad,grade,up_num,up_adress,youtube) "
				+ "VALUES (?,?,?,?,?,0,0,0,0,0,0, ?,?)";
	...
	private Board4DTO selectByNum(int num, Connection conn) {
		Board4DTO dto = null;

		PreparedStatement pstmt = null;
		ResultSet rs = null;

		String sql = "SELECT * FROM tbl_final4 WHERE num = ?";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public Board4DTO read(int num, String ipAddress) {
		Board4DTO dto = null;
	...
	private void incrReadCnt(int num, Connection conn) {
		...
		String sql = "UPDATE tbl_final4 SET readcnt = readcnt+1 WHERE num = ?";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public void update(Board4DTO dto) {
		...
		String sql = "UPDATE tbl_final4 SET title = ?, content = ?, youtube=? WHERE num = ?";
	...
	public void delete(int num) {
		...
		String sql = "DELETE tbl_final4 WHERE num = ?";
	...
	public int writeReply(int orgNum, Board4DTO dto) {
		int crtdNum = -1;

		Connection conn = null;
		PreparedStatement pstmt = null;

		String sql = "INSERT INTO tbl_final4 "
		...
		try {
			...
			Board4DTO orgDTO = selectByNum(orgNum, conn);
	...
	private void incrRepStep(Connection conn, Board4DTO orgDTO) {
		PreparedStatement pstmt = null;

		String sql = "UPDATE tbl_final4 SET repstep = repstep+1 WHERE reproot = ? AND repstep > ?";
		try {
			pstmt = conn.prepareStatement(sql);
	...
	public void good(int num) {
		...
		String sql = "UPDATE tbl_final4 SET good = good+1 WHERE num = ?";
		try {
			conn = dataFactory.getConnection();
	...
	public void bad(int num) {
		...
		String sql = "UPDATE tbl_final4 SET bad = bad+1 WHERE num = ?";
		try {
			conn = dataFactory.getConnection();
	...
	public String download(int num) {
		...
		String sql = "SELECT up_adress FROM tbl_final4 WHERE num = ?";
		try {
			conn = dataFactory.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public List<Board4DTO> mainList() {
		...
		String sql = "SELECT * FROM (SELECT rownum rnum,num,title,content,writer,to_char(writedate, 'yyyy/mm/dd')"
				...
				+ "(select * from tbl_final4 ORDER BY readcnt DESC)) WHERE rnum BETWEEN 1 AND 5";
		...
		try {
			...
			while (rs.next()) {
				...
				Board4DTO dto = new Board4DTO(num, title, content, writer, writedate, readcnt, repRoot, repStep,
	...
	public void idNumGood(String id, int num) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "select * from tbl_idnumgood4 where id=? and contentnum=?";
		ResultSet rs = null;
	...
	private void recordgood(String id, int num) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "insert into tbl_idnumgood4 (id, contentNum) values(?,?)";
	...
	public void idNumBad(String id, int num) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "select * from tbl_idnumbad4 where id=? and contentnum=?";
		ResultSet rs = null;
	...
	private void recordbad(String id, int num) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "insert into tbl_idnumbad4 (id, contentNum) values(?,?)";
	...
	private void ipcheck(Connection conn, int num, String ipAddress) {
		...
		String sql = "Select * from tbl_ipcount4 where ip=? and ipcontentnum=? and ipdate=?";
	...
	private void writeipinfo(int num, String ipAddress, String ipdate) {
		...
		String sql = "insert into tbl_ipcount4 (ip, ipcontentNum, ipdate) values(?, ?, ?)";
	...
	public boolean checkGood(int num, String loginedId) {
		...
		String sql="select * from tbl_idnumgood4 where contentnum = ? and id = ?";
		try {
			conn = dataFactory.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
